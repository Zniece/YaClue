...
也可以直接执行 `cargo build -p app` 编译 Rust 应用成员。当前界面用于正式 GUI 设计前的后端全链路验证。普通数学输入框一次接受一个 YaClue 数学表达式，不接受 Yacas 的定义、赋值或多语句脚本；方程组由界面按行拆分。编写或测试这些形式时，请使用 Yacas 脚本语言和 `.ys` 文件。
