...
> **Running examples:** `In>` and `Out>` denote input and output. `yacas-rs` does not provide the historical interactive console as a separate program. Run these expressions through YaClue’s direct-evaluation view or a test host.
...
`ArcSin is sometimes used. The underscore character ` is a
...
The `D` function also accepts an argument specifying how many times the
...
the function [N](../reference/arithmetic.md#nexpression), then *Yacas will do so*, as for example in
`N(Sqrt(2),50)`.
...
returning boolean values are called *predicates*. For example, [IsNumber](../reference/predicates.md#isnumberexpr)
and `IsInteger` are predicates defined in the Yacas environment. For
example, try `IsNumber(2+x)`, or `IsInteger(15/5)`.

There are also comparison operators. Typing `2 > 1` would return `True`.
...
language [LISP](https://en.wikipedia.org/wiki/Lisp_(programming_language)) developed in the 1970's. From a small set of operations
...
n*f(n-1);`` Now we realize we need a function `IsGreaterThanZero`, so we
...
standard library and it's called [IsPositiveInteger](../reference/predicates.md#ispositiveintegern), so it was not
...
$f(n)=n(n-1)\ldots 1$. The rules are given precedence values 10 and
...
the function object denotes a *postpredicate* that should return `True` or
...
result with the function [Echo](../reference/io.md#echoitem):
...
`a`, then the [Echo](../reference/io.md#echoitem) command, and finally evaluates `1+2`, and
...
the function [Apply](../reference/controlflow.md#applyfn-arglist) as follows:
...
arguments in to an expression (they should be the respective [identity elements](http://en.wikipedia.org/wiki/Identity_element), hence it
...
`For` function already, but this is how it could be defined in one line
(In Yacas the `For` function is bodied, we left that out here for clarity,
...
up a macro named `for` with four arguments. On the right is the body
...
You can also use the back-quote ```` to expand a macro in-place. It takes on the form ``(expression)``, where the expression can again
...
In this project, expressions can be evaluated through YaClue's development
interface or a Rust test host. Longer programs should be kept in `.ys` files
and loaded into an initialized environment. See [Getting
Started](../getting-started/index.md) for the current workflow.
...
such as `xx` or `F or even -+@+-_`.  Apart from possibly
...
The imaginary unit $\imath$ is denoted `I` and complex numbers can be
entered as either expressions involving `I`, as for example
`1+I*2`, or explicitly as `Complex(a,b)` for $a+\imath b$. The form
