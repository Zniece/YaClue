# Programming in the Yacas Scripting Language

This guide explains the Yacas scripting language and its current Rust environment. It assumes that you have
...
Yacas uses a small Rust core to interpret an external library of scripts. The
core provides the parser, evaluator, rule system, numeric primitives and commands
that require direct access to the environment. The script library resides in
`yacas/scripts` and defines higher-level functions and constants. Library
scripts use the same language and rule mechanism as application extensions.

Core commands are implemented by the modules under
`yacas/yacas-rs/src/commands` and registered by
`commands::register_core_commands`; this registration is the authoritative
implementation list. Many user-facing operators are script rules over lower-level
core commands. For instance, the
...
When a complete Yacas environment starts, it executes the script
...
The Rust engine also provides environment-owned arrays and associations. They
are exposed through the historical generic-object predicates for script
compatibility, but their implementation is part of `yacas-rs`; scripts do not
load arbitrary native object implementations.
...
one rule (with the predicate `True`), but it is a desired behavior for
...
another with precedence 20 and the predicate that returns `True` only
...
The Yacas scripting language allows to have rules whose definitions are
...
`False` since {x} is an atom, so we would have defined the predicate
to be always `False`, which is not at all what we meant to do. For
...
will evaluate to `False` and the new rule will be defined with a
predicate that is always `False`, i.e. it will be never applied.
...
back-quoted by putting a `\`` in front of it. Within the back-quoted expression, all atoms that have a `@` in front of them get replaced with the value of that atom (treated as a variable), and then the resulting expression is evaluated: ::

  In> x:=y
  Out> y
  In> `(@x:=2) Out> 2 In> x Out> y In> y Out> 2
...
Yacas has the function [FullForm](../reference/io.md#fullformexpr) to show the internal representation:
...
The internal representation is very close to what [FullForm](../reference/io.md#fullformexpr) shows
...
afterwards. This post-predicate must also evaluate to `True`.  If the
structure of the expression matches the structure of the pattern, and
all predicates evaluate to `True`, the pattern matches and the
...
use [FlatCopy](../reference/lists.md#flatcopylist) to actually make a new copy of an object.  Another
...
working on arguments, e.g. [Cos](../reference/elementary.md#cosx) or `*`, will always have the same
...
should hold for the [Cos](../reference/elementary.md#cosx) function, regardless of the type of its
argument.
...
1. $a*b$ (if $b$ is a vector and $a$ is a number) `<--`
   return vector of each component multiplied by $a$.
2. $a*0$ `<--` $0$
...
* If {InNumericMode()} returns `True` and the arguments are numbers
...
{InNumericMode()} returns `True`, and otherwise returns unevaluated.

The "symbolic" function usually has multiple rules while the
"numerical" function is usually just one large block of
number-crunching code.

### Using [N](../reference/arithmetic.md#nexpression) and [InNumericMode](../reference/numeric-programming.md#innumericmode) in scripts

As a rule, [N](../reference/arithmetic.md#nexpression) should be avoided in code that implements basic
numerical algorithms. This is because [N](../reference/arithmetic.md#nexpression) itself is implemented in
...
[MathDivide](../reference/core-functions.md#mathdivide), [MathSin](../reference/core-functions.md#mathsin) and so on, without using [N](../reference/arithmetic.md#nexpression).
For example, if your code needs to evaluate $\sqrt{\pi}$ to many
digits as an intermediate result, it is better to write
`MathSqrt(Internal'Pi())` than `N(Sqrt(Pi))` because it makes for faster,
more reliable code.

### Using [Builtin'Precision'Set](../reference/numeric-programming.md#builtinprecisionsetn)
...
convention is that if [InVerboseMode](../reference/io.md#inverbosemode) returns `True`, functions may
print diagnostic information. (But do not print too much!). Verbose
mode is turned on by using the function [V](../reference/io.md#vexpression). The
...
$n!! := n(n-2)\ldots 1$ written using predicates and rules:
...
(The library may use a different implementation.) Separate rules often state
the applicable cases more directly and can reject candidates before entering a
larger body. Neither form is universally faster: predicate cost, rule count and
argument shape should be measured on representative expressions.

Recursion can make a symbolic definition clearer, but it consumes evaluation
depth. The Rust engine starts with a maximum depth of 600 and allows the host or
`MaxEvalDepth` to change it. Use recursion for naturally shrinking symbolic
structures and bounded loops for large numerical workloads.

### Reporting errors

Core commands should report invalid argument types. Script rules may instead
remain inapplicable and leave a symbolic call unevaluated when that is the
...
error messsage. The Rust evaluator reports this as a structured `YacasError` and unwinds the
Rust call stack. Script authors should still restore deliberately changed global
mode or precision state when a computation can fail. Use the trapping functions
when an error is expected and evaluation should continue.
...
[MathAdd](../reference/core-functions.md#mathadd), however, is a function defined in the *core* to performs
the numeric addition. It can only do this if the arguments are numeric
and it fails on symbolic arguments.  (The `+` operator calls [MathAdd](../reference/core-functions.md#mathadd)
after it has verified that the arguments passed to it are numeric.)

A core function such as [MathAdd](../reference/core-functions.md#mathadd) can never return unevaluated, but an
...
Here we made a typo: we should have written [MathAdd](../reference/core-functions.md#mathadd), but wrote
[Mathadd](../reference/core-functions.md#mathadd) instead. Yacas` happily assumed that we mean a new and (so far) undefined *user function* [Mathadd](../reference/core-functions.md#mathadd) and returned the expression unevaluated.
...
particular argument (using [HoldArg](../reference/programming-language.md#holdargoperator-parameters) or [HoldArgNr](../reference/programming-language.md#holdargnrfunction-arity-argnum)).
...
make this argument unevaluated by putting [Hold](../reference/controlflow.md#holdexpr) on it and always
...
If we define a rule base for `A` and declare [HoldArg](../reference/programming-language.md#holdargoperator-parameters), :
...
[HoldArg](../reference/programming-language.md#holdargoperator-parameters) declaration. So in the body, `x` will just be the atom `x`,
...
performed using [Eval](../reference/controlflow.md#evalexpr), or with backquoting, or by using a core
...
(using a core function [MacroSet](../reference/programming-language.md#macroset) that evaluates its first argument).
...
### Special behavior of [Hold](../reference/controlflow.md#holdexpr), [UnList](../reference/lists.md#unlistlist) and [Eval](../reference/controlflow.md#evalexpr)
...
> **Current boundary:** Rust tests cover `CustomEval`, `TraceExp`, and the script-level `TraceRule`. `TraceStack` currently evaluates its expression without recording call history. The interactive `Debug>` examples below depend on an unavailable historical console hook and explain the mechanism rather than current usage. See the [debugging reference](../reference/debugging.md) for current behavior.
...
The Rust engine uses the same evaluator in debug and release builds. Rust debug
builds add ordinary assertions and lower optimization, but they do not expose a
different Yacas tracing contract.
...
The maintained function reference records which trace facilities are currently implemented.
...
{x} is set to {2}, so we expect {IsNumber} to return `True`. Stepping
again: :
...
Suppose we want to define special symbols $A(k)$ and $B(k)$
that we can multiply with each other or by a number, or add to each other,
but not commute with each other, i.e. $A(k)B(k)\ne B(k)A(k)$. Here
$k$ is merely a label to denote that $A(1)$ and $A(2)$
...
$A(k)$ and $B(k)$; between usual numbers this would be the same
as normal multiplication. The symbols $A(k)$, $B(k)$ will never
...
*c-numbers* while non-commuting objects made up of $A(k)$ and
$B(k)$ are called *quantum quantities* or *q-numbers*.)
There are certain commutation relations for these
symbols: the $A$'s commute between themselves, $A(k)A(l) = A(l)A(k)$, and also the $B$'s, $B(k)B(l) = B(l)B(k)$.
However, the $A$'s don't commute with the $B$'s:
$A(k)B(l) - B(l)*A(k) = \delta(k-l)$. Here the $\delta$ is a
*classical* function (called the *Dirac* $\delta$ *function*)
...
expression containing $A$'s and $B$'s will compute something
...
e.g. $\mathrm{VEV}(4) = 4$,
$\mathrm{VEV}(\delta(k-l)) = \delta(k-l)$; and
$\mathrm{VEV}(X*A(k)) = 0$, $\mathrm{VEV}(B(k)*X) = 0$ where
$X$ is any expression, commutative or not. It is straightforward to
compute VEV of something that contains $A$'s and $B$'s: one
just uses the commutation relations to move all $B$'s to the left of
all $A$'s, and then applies the definition of VEV, simply throwing
...
will not be possible to divide anything by $A(k)$ or $B(k)$
-- such division is undefined.

A possible canonical form for expressions with $A$'s and $B$'s
...
and no $B$'s. When encountering a term such as `A(k) ** X`, the routine
...
Things now work as expected. Yacas's [Simplify](../reference/simplify.md#simplifyexpr) facilities can be
