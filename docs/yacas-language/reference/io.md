...
internally represented. In the second example, $2i$ is
first evaluated to `Complex(0,2)` before the expression
is printed.

> **See also:** [LispRead](io.md#lispread), [Listify](lists.md#listifyexpr), [Unlist](lists.md#unlistlist)
...
printed, followed by a newline.    {Echo} always returns `True`.
...
> **See also:** [PrettyForm](io.md#prettyformexpr), [Write](io.md#writeexpr), [WriteString](io.md#writestringstring), [RuleBaseListed](programming-language.md#rulebaselistedname-params)
...
[PrettyForm](io.md#prettyformexpr) renders an expression in a nicer way, using ascii art.
...
> **See also:** [EvalFormula](io.md#evalformulaexpr), `PrettyPrinter'Set`
...
Show an evaluation in a nice way, using `PrettyPrinter'Set`  to show
'input = output'.
...
> **See also:** [PrettyForm](io.md#prettyformexpr)
...
[TeXForm](io.md#texformexpr) returns a string containing LaTeX representation of the Yacas
...
[CForm](io.md#cformexpr) returns a string containing C code that attempts to
...
[IsCFormable](io.md#iscformableexpr) returns `True` if the Yacas expression `expr` can
be exported  into C code. This is a check whether the C exporter [CForm](io.md#cformexpr)
...
(e.g. [UnList](lists.md#unlistlist) cannot be exported). All variables and constants are
considered exportable. The verbose option prints names of functions that
are not exportable. The second calling format of [IsCFormable](io.md#iscformableexpr) can be used to
...
This returned `False` because the function `func123` is not
...
> **See also:** [CForm](io.md#cformexpr), [V](io.md#vexpression)
...
that [Write](io.md#writeexpr) accepts an arbitrary number of arguments, all of which are
written to the current output (see second example). [Write](io.md#writeexpr) always
returns `True`.
...
the output of [Write](io.md#writeexpr).

> **See also:** [Echo](io.md#echoitem), [WriteString](io.md#writestringstring)
...
[WriteString](io.md#writestringstring) always returns `True`.
...
This example clearly shows the difference between [Write](io.md#writeexpr) and
[WriteString](io.md#writestringstring). Note that [Write](io.md#writeexpr) and [WriteString](io.md#writestringstring) do not
write a newline, so the `Out>` prompt immediately follows the output.

> **See also:** [Echo](io.md#echoitem), [Write](io.md#writeexpr)
...
[Space](io.md#space) prints one space on the  current output. The second form prints
`n` spaces on the current  output. The result is always `True`.
...
> **See also:** [Echo](io.md#echoitem), [Write](io.md#writeexpr), [NewLine](io.md#newline)
...
[NewLine](io.md#newline) prints a newline character on the current output. The second
...
> **See also:** [Echo](io.md#echoitem), [Write](io.md#writeexpr), [Space](io.md#space)


### bodied FromFile(body, name)
...
> **See also:** `ToFile`, `FromString`, [Read](io.md#read), [ReadToken](io.md#readtoken)


### bodied FromString(body, str)
...
> **See also:** `ToString`, `FromFile`, [Read](io.md#read), [ReadToken](io.md#readtoken)


### bodied ToFile(body, name)
...
contents will be overwritten.  This is a limitation of `ToFile`: one
...
> **See also:** `FromFile`, `ToString`, [Echo](io.md#echoitem), [Write](io.md#writeexpr), [WriteString](io.md#writestringstring), [PrettyForm](io.md#prettyformexpr), [Taylor](calc.md#taylorvar-at-order-expr)


### bodied ToString(body)
...
[Echo](io.md#echoitem) for instance, is collected in a string and this string is
returned.
...
> **See also:** `FromFile`, `ToString`, [Echo](io.md#echoitem), [Write](io.md#writeexpr), [WriteString](io.md#writestringstring)
...
> **See also:** `FromFile`, `FromString`, [LispRead](io.md#lispread), [ReadToken](io.md#readtoken), [Write](io.md#writeexpr)


### bodied ToStdout(body)
...
`ToStdout` can be used to select this stream.

### LispRead()

read expressions in LISP syntax

[LispRead](io.md#lispread) reads an expression in the LISP syntax from the current
input, and returns  it unevaluated. When the end of an input file is
encountered, the  special token atom `EndOfFile` is returned. The Yacas
...
> **See also:** `FromFile`, `FromString`, [Read](io.md#read), [ReadToken](io.md#readtoken), [FullForm](io.md#fullformexpr), [LispReadListed](io.md#lispreadlisted)
...
[LispReadListed](io.md#lispreadlisted) reads a LISP expression and returns it in a list,
instead of the form usual to Yacas (expressions). The result can be thought
of as applying [Listify](lists.md#listifyexpr) to [LispRead](io.md#lispread).  The function
[LispReadListed](io.md#lispreadlisted) is more useful for reading arbitrary LISP expressions,
...
> **See also:** `FromFile`, `FromString`, [Read](io.md#read), [ReadToken](io.md#readtoken), [FullForm](io.md#fullformexpr), [LispRead](io.md#lispread)
...
> **See also:** `FromFile`, `FromString`, [Read](io.md#read), [LispRead](io.md#lispread), [DefaultTokenizer](io.md#defaulttokenizer)
...
evaluated. [Load](io.md#loadname) always returns `True`.

> **See also:** [Use](io.md#usename), [DefLoad](io.md#defloadname), `DefaultDirectory`, [FindFile](io.md#findfilename)
...
[Use](io.md#usename) or via the [DefLoad](io.md#defloadname) mechanism, nothing happens. Otherwise
all expressions in the file are  read and evaluated. [Use](io.md#usename) always
returns `True`. The purpose of this function is to make sure that the
file will at least have been loaded, but is not loaded twice.

> **See also:** [Load](io.md#loadname), [DefLoad](io.md#defloadname), `DefaultDirectory`
...
> **See also:** [Load](io.md#loadname), [Use](io.md#usename), `DefaultDirectory`
...
> **See also:** [Load](io.md#loadname), `DefaultDirectory`
...
[PatchLoad](io.md#patchloadname) loads in a file and outputs the contents to the current
...
> **See also:** [PatchString](strings.md#patchstringstring), [Load](io.md#loadname)
...
> **See also:** [NewLine](io.md#newline)
...
[V](io.md#vexpression) will evaluate the expression in verbose mode. Various parts of
Yacas can show extra information about the work done while doing a
calculation when using [V](io.md#vexpression). In verbose mode, [InVerboseMode](io.md#inverbosemode) will
...
In verbose mode, [InVerboseMode](io.md#inverbosemode) will return `True`, otherwise it
will return `False`.
...
> **See also:** [XmlTokenizer](io.md#xmltokenizer)
...
> **See also:** [OMRead](io.md#omread), [TrapError](errors.md#traperrorexpressionerrorhandler), [XmlExplodeTag](io.md#xmlexplodetagxmltext), [ReadToken](io.md#readtoken), `FromFile`, `FromString`
...
> **See also:** [XmlTokenizer](io.md#xmltokenizer), [XmlExplodeTag](io.md#xmlexplodetagxmltext), [OMDef](io.md#omdefyacasform-cd-name)
