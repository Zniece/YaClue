...
Detailed material includes the [programming guide](programming/index.md), [function reference](reference/index.md), [algorithm notes](algorithms/index.md), [tutorial](tutorial/index.md), and [glossary](glossary.md). This material originated in Yacas documentation and is now maintained with this implementation. Historical coverage can exceed product capabilities; tests and compatibility policy determine guarantees. See the [license](license.md) and [credits](credits.md) for attribution.
...
| Desktop application | `app/` | Product interaction; it does not define Yacas semantics |
...
- Do not turn implementation details into language guarantees; keep measurements in development status notes.
- Use historical implementations for attribution and investigation, not as automatic authority over current behavior.
