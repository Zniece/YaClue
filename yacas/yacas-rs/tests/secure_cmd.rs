...
fn secure_frame() {
    ...
    let probes = [
        (r#"TrapError(Secure(SystemCall("echo hi")), "caught")"#, "\"caught\""),
        (r#"Secure(1+1)"#, "2"),
        (r#"TrapError(Secure(ToFile("/tmp/sec_t.txt")WriteString("x")), "caught")"#, "\"caught\""),
        (r#"SystemCall("echo ok > /dev/null")"#, "True"),
        (r#"TrapError(Secure([SystemCall("echo a");SystemCall("echo b");]), "caught")"#, "\"caught\""),
        (r#"TrapError(Secure(42), "nope")"#, "42"),
        // 嵌套恢复:Secure 错误后 secure 复位,体外 SystemCall 仍正常
        (r#"[TrapError(Secure(SystemCall("x")), "c"); SystemCall("echo ok > /dev/null");]"#, "True"),
    ];
    for (p, exp) in probes {
