...
fn scattered_probe() {
    ...
    let probes: [(&str, &str); 44] = [
        ...
        (
            "FromBase(10,\"0.2222222222222222222\")",
    ...
    // —— 长分数串探针(与上表分开,便于定位)——
    assert_eq!(
        run(&mut env, "ToBase(2,0.1)"),
        "\"0.000110011001100110011001100110011\""
    );
    assert_eq!(
        run(&mut env, "ToBase(3,0.5)"),
        "\"0.1111111111111111111111111111111111\""
    );
    assert_eq!(
        run(&mut env, "ToBase(5,0.3)"),
        "\"0.1222222222222222222222222222222222\""
    );
    // —— 系统信息/状态族 ——
    ...
    assert_eq!(
        run(&mut env, "DebugFile(1)"),
        "ERR(Generic(\"Cannot call DebugFile in non-debug version of Yacas\"))"
    );
    assert_eq!(
        run(&mut env, "DebugLine(1)"),
        "ERR(Generic(\"Cannot call DebugLine in non-debug version of Yacas\"))"
    );
    // FindFile 裸名优先命中 = CWD 相对(oracle 探针 cwd=scripts);测试切 cwd 对齐
    let old_cwd = std::env::current_dir().unwrap();
    std::env::set_current_dir(d).unwrap();
    assert_eq!(
        run(&mut env, "FindFile(\"yacasinit.ys\")"),
        "\"yacasinit.ys\""
    );
    ...
    assert_eq!(
        run(&mut env, "FindFunction(\"MathSqrt\")"),
    ...
    assert_eq!(
        run(&mut env, "FromString(\"1;\")CurrentFile();"),
        "\"String\""
    );
    // —— XmlExplodeTag ——
    assert_eq!(
    ...
    assert_eq!(
        run(
            &mut env,
            "FromString(\"<a> x <b>\")[XmlTokenizer();ReadToken();];"
        ),
        "<a>"
    );
    assert_eq!(
        run(
            &mut env,
            "FromString(\"<a> x <b>\")[XmlTokenizer();ReadToken();ReadToken();];"
        ),
        " x "
    );
    assert_eq!(
        run(
            &mut env,
            "FromString(\"1+2;\")[DefaultTokenizer();Read();];"
        ),
        "1+2"
    );
...
fn trace_rule_output() {
    ...
    let t = parse_expression(&mut env, &format!("{src};"))
        .unwrap()
        .expect("非空");
    ...
    assert!(
        lines[0]
            .trim_start()
            .starts_with("TrEnter(\"g\",\"g(3)\",\"\",0);"),
        "{text}"
    );
    assert!(
        lines[1].trim_start().starts_with("TrArg(\"3\",\"3\");"),
        "{text}"
    );
    assert!(
        lines[2]
            .trim_start()
            .starts_with("TrLeave(\"g(3)\",\"6\");"),
        "{text}"
    );
