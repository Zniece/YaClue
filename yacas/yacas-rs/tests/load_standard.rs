...
fn boot(env: &mut Environment) {
    ...
    let stdarith_ys = concat!(env!("CARGO_MANIFEST_DIR"), "/../../yacas/scripts/stdarith.ys");
    ...
    let stubs_ys = concat!(env!("CARGO_MANIFEST_DIR"), "/../../yacas/scripts/stubs.rep/code.ys");
...
fn nrargs_and_normalform() {
    ...
    assert!(matches!(err_of(&mut env, "NrArgs(x)"), YacasError::InvalidArg));
