...
fn step3_battery() {
    ...
    let probes: Vec<&str> = vec![
        ...
        "N(Cos(1),10)", "N(Tan(1),10)", "N(Sqrt(2),10)", "N(Sqrt(2),20)",
        "N(Exp(1),10)", "N(Exp(1),20)", "N(Ln(2),10)", "N(Ln(2),20)",
        "N(1/3,10)", "N(2/3,10)", "N(2/3,20)",
        "N(Pi,10)", "N(Pi,15)", "N(Pi,20)", "N(Pi,25)", "N(Pi,30)",
        "Div(7,3)", "Div(-7,3)", "MathDiv(-7,3)",
        "Rem(7,3)", "Rem(-7,3)", "Rem(10,5)",
    ];
    let expect = [
        "0.8414709848", "0.841470984807", "0.841470984807896", "0.84147098480789650665",
        "0.5403023058", "1.5574077246", "1.4142135623", "1.4142135623730950488",
        "2.7182818284", "2.71828182845904523536", "0.6931471806", "0.69314718055994530942",
        "0.3333333333", "0.6666666666", "0.66666666666666666666",
        "3.141592653589793", "3.14159265358979323846", "3.1415926535897932384626433",
        "3.141592653589793238462643383279", "3.14159265358979323846264338327950288",
        "2", "-2", "-2",
        "1", "-1", "0",
    ];
    ...
    for (p, e) in probes.iter().zip(expect.iter()) {
        ...
        if KNOWN.contains(p) {
            ...
            let close = dg(&got) == dg(e) && got.starts_with(&e[..e.len().saturating_sub(2)].to_string());
            if !close {
                bad += 1;
                println!("REGRESS {p}\n  got  {got}\n  want {e}");
            } else {
