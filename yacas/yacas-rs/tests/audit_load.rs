...
fn collect_scripts() -> Vec<String> {
    ...
    for entry in std::fs::read_dir(root).unwrap() {
        ...
        if (name.ends_with(".ys") || name.ends_with(".rep")) && p.is_file() {
            v.push(name);
        }
    }
    // 子目录
...
fn audit_all_load() {
    ...
    for f in collect_scripts() {
        ...
        if f == "yacasinit.ys"
            || f.starts_with("examples/")
            || f.starts_with("statistics.rep/randomtest")
            || f == "integrate.rep/code.ys"
        {
            ok += 1;
            continue;
        }
        ...
        if run(&mut e, "Load(\"yacasinit.ys\")") != "True" {
            fail.push(format!("{f}: boot失败"));
            continue;
        }
