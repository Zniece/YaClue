...
| `"spoken-ssml-with-highlighting"`| Like `"spoken-ssml"` but with additional annotations necessary for synchronized highlighting (read aloud). |
   * @category Mathfield
   */
export type OutputFormat = 'ascii-math' | 'yaclue' | 'latex' | 'latex-expanded' | 'latex-unstyled' | 'latex-without-placeholders' | 'typst' | 'math-ml' | 'plain-text' | 'spoken' | 'spoken-text' | 'spoken-ssml' | 'spoken-ssml-with-highlighting';
export type YaClueDiagnosticCode = 'empty-expression' | 'mathlive-error' | 'missing-slot' | 'ambiguous-semantics' | 'unsupported-atom' | 'unsupported-command' | 'invalid-argument' | 'invalid-assumption' | 'lossy-conversion';
export interface YaClueDiagnosticLocation {
    path: readonly (string | number)[];
    atomType?: string;
    command?: string;
}
export interface YaClueDiagnostic {
    severity: 'error' | 'warning';
    code: YaClueDiagnosticCode;
    message: string;
    location?: YaClueDiagnosticLocation;
}
export type YaClueConversionResult = {
    ok: true;
    value: string;
    diagnostics: YaClueDiagnostic[];
} | {
    ok: false;
    diagnostics: YaClueDiagnostic[];
};
export type YaClueAssumptionFact = 'positive' | 'negative' | 'non_zero';
export interface YaClueAssumption {
    symbol: string;
    fact: YaClueAssumptionFact;
}
export type YaClueSubmissionResult = {
    ok: true;
    expression: string;
    assumptions: YaClueAssumption[];
    diagnostics: YaClueDiagnostic[];
} | {
    ok: false;
    diagnostics: YaClueDiagnostic[];
