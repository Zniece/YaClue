const content = {
  "zh-CN": [
    ...
    { title: "使用数学键盘", text: "键盘顶部的分类标签可以左右滑动，点击标签切换基础、微积分、矩阵等页面。每页右侧的数字、方向键、删除键和 ↵ 都在相同位置。方向键移动光标，↵ 提交计算；键盘上方控制条的箭头可收起或展开键盘。较矮的横屏中，提交后键盘会自动收起。Mat 插入可调整大小的矩阵，+Row 和 +Col 用于增加行列。", examples: [] },
